...
def createMinizinc(dimension_size_list,numSam,bounds,directory):
    
    file = open(directory+'model.mzn', 'w')
    
    for i,size in enumerate(dimension_size_list):
        file.write("int: var_"+str(i)+";\n")
        file.write("set of int: var"+str(i)+" = 1..var_"+str(i)+";\n")
    file.write("int: n_bounds;\n")
    file.write("set of int: bounds = 1..n_bounds;\n")
    file.write("array [bounds] of set of int: bounds_sets;\n")
    
    
    
    rows_length=1
    for size in dimension_size_list:
        rows_length=rows_length*size
    rows=list(range(rows_length))
        
        
    repeatDim = ()
    r = set([v for v in range(len(dimension_size_list))])
    keys=[]
...
def generatesSample(num_nurses,num_days,num_shifts,numSam,bounds,directory):
#    print(bounds)
...
def generateSample(num_nurses,num_days,num_shifts,numSam,nurse_skill,bounds,subset1_bounds,subset2_bounds,directory,bk):
#    print(bounds)
